...
local testItem = class("testItem", item)
testItem.initialize = function(self, name)
	self.name = name
end
testItem.equals = function(self, other)
	return self.name == other.name
end
testItem.__tostring = function(self)
	return self.name
end
...
local testProvider = class("testProvider", provider)
testProvider.initialize = function(self) end
testProvider.canProvide = function(self, item, qty)
	if item.name == "wood" then return qty end
	return 0
end
testProvider.reserve = function(self, item, qty)
	local function cancel() end
	local function perform(dstNodeName, dir, onDone)
		table.insert(queue, function(fm)
			print("provider sent " .. tostring(item) .. "*" .. qty .. " to right")
			fm:route(item, qty, "N1", dstNodeName, dir, onDone)
		end)
	end
	return {cancel = cancel, perform = perform}
end
testProvider.priority = function(self, item, qty) return 0 end

fm:addProvider(testProvider:new())

------------
--testSink--
------------

local testSink = class("testSink", sink)
testSink.initialize = function(self) end
testSink.canSink = function(self, item, qty)
	return qty
end
testSink.sink = function(self, item, qty)
	return {dstNodeName = "N1", dir = "up", qty = qty, onDone = function(qty)
		print("sink got " .. tostring(item) .. "*" .. qty)
	end}
end
testSink.priority = function(self, item, qty) return 0 end

fm:addSink(testSink:new())

------------
--testNode--
------------

local testNode = class("testNode", routable)
testNode.initialize = function(self, name)
	self.name = name
end
testNode.route = function(self, item, qty, dir, onDone)
	table.insert(queue, function()
		print(self.name .. " sent " .. tostring(item) .. "*" .. qty .. " to " .. dir)
		onDone(qty)
	end)
end
testNode.setUnexpectedHandler = function(self, onUnexpected) end

fm:addNode("N1", testNode:new("N1"))
fm:addNode("N2", testNode:new("N2"))
fm:addNode("N3", testNode:new("N3"))
fm:addRoute("N1", "down", "N2")
fm:addRoute("N2", "down", "N3")
fm:addRoute("N3", "up", "N2")
fm:addRoute("N2", "up", "N1")

---------------
--testCrafter--
---------------
local testCrafter = class("testCrafter", crafter)
testCrafter.initialize = function(self, name, result, resultQty, materials, inputNode, inputDir, outputNode, outputDir)
	self.name = name
	self.result = result
	self.resultQty = resultQty
...
testCrafter.getResult = function(self)
	return self.result
end
testCrafter.numResult = function(self)
	return self.resultQty
end
testCrafter.getMaterials = function(self)
	local rst = {}
	for k, v in pairs(self.materials) do
		local entry = {}
		entry.item = v
		entry.dstNodeName = self.inputNode
		entry.dir = self.inputDir
		rst[k] = entry
	end
	return rst
end
testCrafter.craft = function(self)
	return {
		onMaterialDelivered = function(materialIndex)
			print(self.name .. " got " .. tostring(self.materials[materialIndex]) .. "*1")
		end,
		onAllMaterialsDelivered = function(destinations)
			local remaining = self.resultQty
			print(self.name .. " sent " .. tostring(self.result) .. "*" .. self.resultQty .. " to " .. self.outputDir)
			for k, v in pairs(destinations) do
				remaining = remaining - v.qty
				fm:route(self.result, v.qty, self.outputNode, v.dstNodeName, v.dir, v.onDone)
			end
			if remaining > 0 then
				fm:sinkItem(self.outputNode, self.result, remaining, function(qtyDone) end)
			end
		end
	}
end
testCrafter.priority = function(self) return 0 end

fm:addCrafter(testCrafter:new("C1", testItem:new("plank"), 4, {testItem:new("wood")}, "N1", "right", "N1", "left"))
fm:addCrafter(testCrafter:new("C2", testItem:new("stick"), 4, {testItem:new("plank"), testItem:new("plank")}, "N2", "right", "N2", "left"))
fm:addCrafter(testCrafter:new("C3", testItem:new("pick"), 1, {testItem:new("plank"), testItem:new("plank"), testItem:new("plank"), testItem:new("stick"), testItem:new("stick")}, "N3", "right", "N3", "left"))

-------
--run--
-------

local rst = fm:planCrafting(testItem:new("pick"), 1, "N3", "left")
print("available qty: " .. rst.qtyAvail)
rst.perform(function(qtyDone)
	print("requester got " .. qtyDone)
end)
while #queue > 0 do
	local peek = queue[#queue]
	table.remove(queue)
	peek(fm)
end
